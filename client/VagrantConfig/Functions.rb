...
$rsync_excludes = [".git/","VagrantConfig/","Vagrantfile","ssh/","server_config.json"]
...
def shell_script_deployment(machine,machine_name,machine_properties)
	...
	script_args='';
	if not deployment_script.kind_of?(String)
		script_args = deployment_script['args'].map { |s| %Q("#{s}") }.join(' ') if deployment_script['args'];
    deployment_script = deployment_script['path'];
	end
  script_name = %Q("#{machine_properties['server_config']['repository_code_folder']}/#{deployment_script}")
	machine.vm.provision "shell_bash", run: "always" do |s|
		...
		s.args = [ script_name, script_args, $project_name_sanitised ]
		s.privileged = false
		s.inline = <<-EOC
			HOME_DIR=$(printf ~);
			DEPLOYMENT_SCRIPT_LOCATION=$HOME_DIR/$3.sh;
